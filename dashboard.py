...
# membuat fungsi untuk membuat rent_byholiday_df
def create_rent_byholiday_df(df):
    rent_byholiday_df = df.groupby(by='holiday').cnt.sum().reset_index()
    map_holiday = {
        0 : 'non holiday',
        1 : 'holiday'
    }
    rent_byholiday_df['holiday'] = rent_byholiday_df['holiday'].map(map_holiday)
    return rent_byholiday_df

# membuat fungsi untuk membuat rent_byday_df
def create_rent_byday_df(df):
    rent_byday_df = df.groupby(by='weekday').cnt.sum().reset_index()
    map_day ={
        0: 'Sun',
        1: 'Mon',
        2: 'Tues',
    ...
    rent_byday_df['weekday'] = rent_byday_df['weekday'].map(map_day)
    return rent_byday_df
...
byholiday_df = create_rent_byholiday_df(main_df)
byday_df = create_rent_byday_df(main_df)
...
with col1:
    ...
    ax.set_title('Number of Rents by Season', loc='center', fontsize=30)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
...
# menampilkan informasi rata-rata sewa sepeda berdasarkan hari dan bulan
...
with col1:
    ...
    ax.set_title('Number of Rents by Day', loc='center', fontsize=40)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
...
# menampilkan informasi sewa sepeda berdasarkan tipe hari dan kondisi cuaca
st.subheader('Day type & Weather Situation')
col1, col2 = st.columns(2)
...
with col1:
    ...
    sns.barplot(
        y='cnt',
        x='holiday',
        data=byholiday_df.sort_values(by='cnt', ascending=False),
        palette=colors,
        ax=ax
    )
    ax.set_title('Non holiday vs holiday', loc='center', fontsize=40)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='x', labelsize=35)
    ax.tick_params(axis='y', labelsize=25)
...
# menampilkan informasi berdasarkan suhu, kelembaban dan kecepatan angin
